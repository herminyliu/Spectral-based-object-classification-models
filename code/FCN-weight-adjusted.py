        ...
        with open(file_path, 'r') as f:
            ...
            for line in lines:
                ...
                if not line.endswith(','):
                    line += ','
                # Load data
                data = np.loadtxt(file_path, delimiter=',')[-512:].astype(np.float32)
...
train_loader = DataLoader(train_dataset, batch_size=my_batch_size, sampler=sampler)
...
timestamp = datetime.now().strftime("%m-%d-%H%M%S") + "weight_adjusted"
